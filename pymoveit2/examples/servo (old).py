...
class PassingControl(Node):
    ...
    def net_wrench_cb(self, msg):
        """
        Callback for the /net_wrench topic. If the arm is moving toward the box (current_step == 2)
        and the force exceeds 72, immediately trigger the grab and move to the next step.
        """
        if self.current_step == 2 and msg.data > 72:
            self.get_logger().info(f"Net force {msg.data} exceeded threshold in step 2. Initiating grab.")
